...
#wandb.init(project="Cross_Decoder_Vol2", name="Cross_Decoder_GPT2")
wandb.init(project="EncoderXDecoder", name="Cross_Decoder_GPT2_LibriSpeech")
wb_config = wandb.config

#################################################### Parameters
corpus_data = "bookcorpus"
steps = 200
batch_size = 16 #687.500 steps
lr = 2e-4
betas = (0.9, 0.98)
eps = 1e-8
weight_decay = 0.01
...
output_dir = '/efs/cvlachos/SpeechBrain_LCM/cross_decoder_lirbrispeech'
...
def evaluate(model, test_loader):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            ...
            #batch = model.tokenize(batch)
            batch = model.tokenize([b["text"].lower() for b in batch])
            
            input_ids, attention_mask = batch.input_ids.to(device), batch.attention_mask.to(device)

            labels = input_ids.to(device)

            outputs = model(input=input_ids, attention_mask=attention_mask, labels=labels)
            loss = outputs.loss
