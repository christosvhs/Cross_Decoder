...
class GPT2EncoderXDecoder(nn.Module):
    ...
    def forward(self, input = None, encoder_attention_mask = None, input_text = None, decoder_attention_mask=None):
        ...
        global reg_loss
        reg_loss = torch.zeros(input_text.shape[0], input_text.shape[1], out_encoder.last_hidden_state.shape[1]).to(self.device)
        ...
        diffs = (reg_loss[:-1] - reg_loss[1:] + delta*(input_text.shape[1]/input_text.shape[1]))/input_text.shape[1]
