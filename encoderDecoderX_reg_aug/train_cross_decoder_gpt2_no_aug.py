...
from modeling_cross_decoder_gpt2_adapters_regLoss_no_aug import GPT2CrossDecoder 
import wandb
...
wandb.init(project="EncoderXDecoder_vol2_full", name="EncoderXDecoder_HuBERT_GPT2_reverse_adapters_unfrozen_full_no_aug")
...
output_dir = '/scratch/cvlachos/SpeechBrain_LCM/encoderXdecoder/hubert_gpt2_reverse_adapters_unfrozen_full_no_aug'
...
def evaluate(model, test_loader):
    ...
    print(f"Eval loss: {final_loss:.4f} || Eval Perplexity: {perplexity:.4f}")
...
for epoch in range(start_epoch, max_epoch):
    ...
    for n, batch in enumerate(train_loader):
        ...
        outputs = model(input=input_data.input_values.to(device), encoder_attention_mask=input_data.attention_mask.to(device), 
