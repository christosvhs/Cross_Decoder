...
import sys
#from modeling_cross_decoder_gpt2 import GPT2CrossDecoder 
from modeling_cross_decoder_gpt2_adapters_regLoss import GPT2CrossDecoder 
import wandb
...
wandb.init(project="EncoderXDecoder_vol2_full", name="EncoderXDecoder_HuBERT_GPT2_reverse_adapters_unfrozen_full_regLoss_Aug")
...
train_librispeech_360 = load_dataset("openslr/librispeech_asr", "clean", split='train.360', cache_dir=data_cache_folder)
train_librispeech_500 = load_dataset("openslr/librispeech_asr", "other", split='train.500', cache_dir=data_cache_folder)
train_librispeech = concatenate_datasets([train_librispeech_100, train_librispeech_360, train_librispeech_500])
train_librispeech_100, train_librispeech_360, train_librispeech_500 = None, None, None
...
dev_librispeech = concatenate_datasets([dev_librispeech_clean, dev_librispeech_other])
dev_librispeech_clean, dev_librispeech_other= None, None
...
def evaluate(model, test_loader):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            ...
            input_text = model.decoder_tokenize([b["text"].lower() for b in batch])


            outputs, reg_loss = model(
                input=input_data.input_values.to(device),
                encoder_attention_mask=input_data.attention_mask.to(device),
                input_text=input_text.input_ids.to(device),
                decoder_attention_mask=input_text.attention_mask.to(device)
            )
    ...
    print(f"Eval loss: {final_loss:.4f} || Eval Perplexity: {perplexity:.4f} || Regularization loss: {reg_loss:.4f}")
...
if "best.pt" in os.listdir(output_dir):
  with open(os.path.join(output_dir, 'best.pt'), "rb") as f:
...
noisifier = AddNoise(
    csv_file="/workspace2/cvlachos/projects/Cross_Decoder/encoderDecoderX_reg_aug/noise_list.csv",
    snr_low=0,
    snr_high=15,
    pad_noise=True,
    normalize=True,
...
for epoch in range(start_epoch, max_epoch):
    ...
    for n, batch in enumerate(train_loader):
        ...
        speech_clean = [b["audio"]["array"] for b in batch]
        speech_augment = []
        text_augment = [b["text"].lower() for b in batch] + [b["text"].lower() for b in batch]


        for i in speech_clean:
          speech_augment.append(noisifier(torch.tensor(i).unsqueeze(0), torch.ones(1))[0].numpy())

        speech_clean += speech_augment

        # torchaudio.save("clean.wav", torch.tensor(speech_clean[1]).unsqueeze(0), 16_000)
        # torchaudio.save("augmented.wav", torch.tensor(speech_clean[3]).unsqueeze(0), 16_000)


        input_data = model.encoder_preprocess(speech_clean)
        input_text = model.decoder_tokenize(text_augment)

        st = n + last_step
        total_steps += 1      

        outputs, reg_loss = model(input=input_data.input_values.to(device), encoder_attention_mask=input_data.attention_mask.to(device), 
                        input_text=input_text.input_ids.to(device), decoder_attention_mask=input_text.attention_mask.to(device))
        loss = (outputs.loss + lambda_v*reg_loss)/accumulations_steps
        loss.backward()
        ...
        if total_steps%steps == 0:
          ...
          if os.path.exists(os.path.join(output_dir, 'last.pt')):
            os.remove(os.path.join(output_dir, 'last.pt'))
          with open(os.path.join(output_dir, 'last.pt'), "wb") as f:
          ...
          if eval_loss < min_eval_loss:
            ...
            if os.path.exists(os.path.join(output_dir, 'best.pt')):
              os.remove(os.path.join(output_dir, 'best.pt'))
            with open(os.path.join(output_dir, 'best.pt'), "wb") as f:
              torch.save(checkpoint, f)
          else :
            count+=1
