...
class GPT2CrossDecoder(nn.Module):
    ...
    def forward(self, input = None, encoder_attention_mask = None, labels = None, decoder_attention_mask=None):
        ...
        out_token_decoder = self.token_decoder(input_ids=labels.clone(), attention_mask=decoder_attention_mask, 
                                               encoder_hidden_states=out_encoder.last_hidden_state, encoder_attention_mask=None)
        input_mask_decoder = torch.zeros_like(labels, dtype = torch.long) + self.tokenizer.convert_tokens_to_ids("<|mask|>")

        out = self.mask_decoder(input_ids=input_mask_decoder, attention_mask=decoder_attention_mask, encoder_hidden_states=out_token_decoder.past_key_values[-1], labels = labels)
...
if __name__ == "__main__":
    ...
    cross_decoder = GPT2CrossDecoder(decoder_name)
