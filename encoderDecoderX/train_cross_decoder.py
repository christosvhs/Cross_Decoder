...
import sys
from modeling_cross_decoder import BERTEncoderXDecoder 
import wandb
...
wandb.init(project="EncoderXDecoder", name="EncoderXDecoder_HuBERT_BERT_Reverse")
...
output_dir = '/scratch/cvlachos/SpeechBrain_LCM/encoderXdecoder/hubert_bert_reverse'
...
#################################################### model loading
model = BERTEncoderXDecoder().to(device)
...
def evaluate(model, test_loader):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            ...
            labels = label_data.input_ids[label_data.input_ids == model.tokenizer.pad_token_id] = -100

            outputs = model(input=input_data.input_values.to(device), encoder_attention_mask=input_data.attention_mask.to(device), 
                        labels=labels.to(device), decoder_attention_mask=label_data.attention_mask.to(device))
            loss = outputs.loss
    ...
    print(f'Eval loss: {total_loss/len(test_loader)} || Eval PL: {np.exp(total_loss/len(test_loader))} ')
    return total_loss/len(test_loader), np.exp(total_loss/len(test_loader))
...
for epoch in range(start_epoch, max_epoch):
    ...
    for n, batch in enumerate(train_loader):
        ...
        input_data = model.encoder_preprocess([b["audio"]["array"] for b in batch])
        label_data = model.decoder_tokenize([b["text"].lower() for b in batch])
        label_data.input_ids[label_data.input_ids == model.tokenizer.pad_token_id] = -100
        labels = label_data
