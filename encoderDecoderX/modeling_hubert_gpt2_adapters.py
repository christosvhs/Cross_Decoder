...
from transformers import AutoFeatureExtractor, AutoConfig, AutoModel, AutoTokenizer, GPT2LMHeadModel, GPT2Model
...
class GPT2EncoderXDecoder(nn.Module):
    ...
    def __init__(self, encoder_name = "facebook/hubert-base-ls960", decoder_name = "gpt2"):
        ...
        self.decoder = GPT2LMHeadModel.from_pretrained('gpt2', config=mask_config).to(self.device)
        ...
        for name, param in self.decoder.named_parameters():   
            if 'wpe' in name or 'wte' in name or 'crossattention' in name or 'ln_cross_attn' in name:
    ...
    def forward(self, input = None, encoder_attention_mask = None, input_text = None, decoder_attention_mask=None):
        ...
        labels = input_text.to(self.device).clone()
        labels[labels == self.tokenizer.pad_token_id] = -100

        out = self.decoder(input_ids=input_text.clone(), attention_mask=decoder_attention_mask, encoder_hidden_states=out_encoder.last_hidden_state, labels = labels)
...
if __name__ == "__main__":
    ...
    data_cache_folder = '/scratch/cvlachos/LibriSpeech/'
    output_dir = '/scratch/cvlachos/SpeechBrain_LCM/encoderXdecoder/hubert_gpt2_baseline_adapters_full'
    ...
    for idx in tqdm(range(len(test_clean))): 
        ...
        hypothesis =  model.generate(input_data.to(device), label_data.input_ids[:,:1].to(device))[0]        
    ...
    print("Baseline Clean WER", clean_error/len(test_clean))
    ...
    for idx in tqdm(range(len(test_other))): 
        ...
        hypothesis =  model.generate(input_data.to(device),
                                      label_data.input_ids[:,:1].to(device)
                                      )[0].replace("[startoftext] ","")
        hypothesis = hypothesis.split("[endoftext]")[0]
    ...
    print("Baseline Other WER", other_error/len(test_other))
...
######################################## Print what weights are randomly initialed and not pre-trained

# pretrained_model = GPT2Model.from_pretrained("gpt2")

# # Load your custom model (make sure it's the same architecture or subclassed from GPT2Model)
# custom_model = GPT2Model(mask_config)  # Or your own subclass
# custom_model.load_state_dict(
#     pretrained_model.state_dict(), strict=False  # This is key
# )
...
#     if name not in pretrained_model.state_dict():
#         # This param does not exist in pretrained
#         randomly_initialized.append(name)
#     else:
#         pretrained_param = pretrained_model.state_dict()[name]
#         if not torch.allclose(param, pretrained_param, atol=1e-5):
#             # Param is different -> likely randomly initialized
#             randomly_initialized.append(name)

# print("Randomly initialized parameters:")
# for name in randomly_initialized:
#     print(name)
