...
class BertSdpaSelfAttention(BertSelfAttention):
    ...
    def forward(
        ...
        if attention_mask is None:
            att_out = torch.matmul(query_layer, key_layer.transpose(-2,-1))/8
            att_out = torch.softmax(att_out, dim=-1)
            att_out = att_out.mean(dim=1)
            to_heat.append(att_out)

            if len(to_heat)==12:
                torch.save(torch.stack(to_heat, dim=0).mean(dim=0), "heat_map_dev.pt")
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if self.config.is_decoder and encoder_hidden_states is not None:
            encoder_batch_size, encoder_sequence_length, _ = encoder_hidden_states[1].size()
...
class BertLMHeadModel(BertPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, init=None):
        ...
        if init:
            self.bert.load_state_dict(init, strict=False)
...
class BERTEncoderXDecoder(nn.Module):
    ...
    def __init__(self, encoder_name = "facebook/hubert-base-ls960", decoder_name = "bert-base-uncased"):
        ...
        encoder_model_weights = transformers.AutoModel.from_pretrained(decoder_name)
        ...
        self.cross_tokenizer = transformers.AutoTokenizer.from_pretrained(decoder_name)
        special = {"additional_special_tokens" : ["[startoftext]", "[endoftext]"]}   
        self.cross_tokenizer.add_special_tokens(special)   
        self.cross_tokenizer.pad_token = "[PAD]"
        self.cross_tokenizer.eos_token = "[endoftext]"

        token_config = transformers.AutoConfig.from_pretrained(decoder_name)
        token_config.is_decoder=True
        self.token_decoder = transformers.BertModel(token_config).to(self.device)
        self.token_decoder.load_state_dict(encoder_model_weights.state_dict(), strict=False)
        self.token_decoder.resize_token_embeddings(len(self.cross_tokenizer))

        mask_config = AutoConfig.from_pretrained(decoder_name)
        mask_config.add_cross_attention=True
        mask_config.is_decoder=True
        self.mask_decoder = BertLMHeadModel(mask_config, encoder_model_weights.state_dict()).to(self.device)
        self.mask_decoder.resize_token_embeddings(len(self.cross_tokenizer))
    ...
    def decoder_tokenize(self, data):
        ...
        return self.cross_tokenizer(data, padding="longest", truncation=True, return_tensors='pt', max_length=512, add_special_tokens=False)
    
    def decode(self, data):        
        return self.cross_tokenizer.batch_decode(data)
    
    def num_parameters(self):
        return self.encoder.num_parameters() + self.token_decoder.num_parameters() + self.mask_decoder.num_parameters()
    ...
    def forward(self, input = None, encoder_attention_mask = None, labels = None, decoder_attention_mask=None):
        ...
        input_mask_decoder = torch.zeros_like(labels, dtype = torch.long) + self.cross_tokenizer.convert_tokens_to_ids("[MASK]")

        last_hidden_states = (out_token_decoder.last_hidden_state, out_encoder.last_hidden_state)

        out = self.mask_decoder(input_ids=input_mask_decoder, attention_mask=decoder_attention_mask, encoder_hidden_states=last_hidden_states, labels = labels)
    ...
    def generate(self, input_txt=None, max_new_tokens = 10):
        ...
        self.mask_decoder.generation_config.pad_token_id = self.cross_tokenizer.eos_token_id
        ...
        with torch.no_grad():
            for _ in range(max_new_tokens):
                ...
                input_decoder = torch.zeros((input_ids.shape[0],1), dtype = torch.long) + self.cross_tokenizer.convert_tokens_to_ids("<|mask|>")

                out = self.mask_decoder.generate(max_new_tokens=1, num_return_sequences=1, input_ids=input_decoder, attention_mask=attention_mask, encoder_hidden_states=last_hidden_state, position_ids=position_ids)
                
                pad_right = torch.ones((input_ids.shape[0], 1), dtype = torch.int)*self.cross_tokenizer.eos_token_id
...
if __name__ == "__main__":
    ...
    with open("encoderXdecoder/toy_batch.pckl", 'rb') as f:
        batch = pickle.load(f)
