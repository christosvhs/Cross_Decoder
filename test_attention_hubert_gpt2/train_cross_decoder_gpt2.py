...
# wandb.init(project="EncoderXDecoder", name="EncoderXDecoder_HuBERT_GPT2_reverse")
...
train_loader = DataLoader(Dataset(dev_librispeech), batch_size=batch_size, shuffle=False, collate_fn=collate_fn)
...
def evaluate(model, test_loader):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            ...
            outputs = model(input=input_data.input_values.to(device), encoder_attention_mask=input_data.attention_mask.to(device), 
                        labels=label_data.input_ids.to(device), decoder_attention_mask=label_data.attention_mask.to(device))
            loss = outputs.loss
    ...
    print(f'Eval loss: {total_loss/len(test_loader)} || Eval PL: {PL/len(test_loader)} ')
    return total_loss/len(test_loader), PL/len(test_loader)
...
if "last.pt" in os.listdir(output_dir):
